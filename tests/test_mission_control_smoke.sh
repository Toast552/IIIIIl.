...
assert(response.body.failure?.run_id === 'run-demo-failed-test', 'missing failed replay failure panel');
...
assert(response.body.replay_comparison?.recovered?.run_id === 'run-demo-recovered-fork', 'missing recovered replay artifact comparison');
